...
\etocsetnexttocdepth{subsubsection} % Tell etoc to look down to subsubsections
\etocsettocstyle{\subsection*{Project Ideas}}{} % Optional header
\etocsetlevel{section}{6}       % "Hide" sections by setting level too high
\etocsetlevel{subsection}{6}    % "Hide" subsections
...
Investigate deep learning model capabilities to detect what was learned and what is a novel observation.
Most deep learning models trained with maximum a posteriori only output a point estimate and not a model uncertainty.
...
\begin{itemize}
  ...
  \item How well can these models represent multiple modes in the posterior distribution.
...
% Leon
\subsubsection{PrivilegedDreamer: Learning to Estimate Unknown Parameterizations on the Fly}
When deployed into the real world, agents trained in simulation have to deal with uncertainties due to unmodeled effects and unknown parameters.
\citet{Byrd2025} are analyzing how the groundbreaking work for the DreamerV2 world model \cite{Hafner2022} can be extended to deal with the uncertainties of Hidden Parameter MDPs.
After reimplementing and testing the proposed algorithm, it is interesting to
\begin{itemize}
  \item Analyze its limits
  \item The proposed method only deals with aleatoric uncertainties of the estimated parameters.
        For an actual deployment, epistemic uncertainties are playing a major role.
        The framework can be extended to also provide a measure for critical situations unseen during training.
\end{itemize}
...
\begin{itemize}
  ...
  \item Given some contact measurement, can we compute a posterior of the robot state? 
...
% Lennard
\subsubsection{Planning in Latent Spaces}
Optimization-based path planners iteratively refine joint angle sequences to generate short, collision-free paths. However, joint angles and robot geometry live in two fundamentally different geometries: the shortest path in configuration space rarely translates to natural motions in the workspace.
Based on the work by Hung et al. \cite{hung2022reaching}, you implement Variational Autoencoders (VAEs) to learn a latent manifold that integrates kinematics, collision avoidance, and custom optimality criteria. We provide simulation and data-generation tools to support your work.
\begin{itemize}
    \item Develop a self-supervised pipeline to train a VAE. Would other generative models be more suitable?
    \item Apply Activation Maximization for planning and inverse kinematics.
    \item Encode diverse optimality criteria. Can we enfore dynamically consistent paths?
\end{itemize}
...
% Lennart
\subsubsection{How to Get Perfect Policies via Reinforcement Learning}
Achieving nines of task success (i.e. going from 90\% to 99\% to 99.9\% success rate and so on) is an open problem in deep reinforcement learning.
Even on continuous control tasks where optimal controllers provably exist, standard deep RL algorithms frequently plateau below perfect performance, or become unstable during extended training.
Recent work has identified potential root causes as optimization pathologies such as plasticity loss or unbounded weight/feature norm growth \citet{lee2025hyperspherical}. 
This project selects a set of benchmark tasks known to be perfectly solvable in theory (e.g. CartPole-Swingup, Pendulum), so that any performance gap can be unambiguously attributed to optimization failure. 
The student systematically combines known interventions and tracks diagnostic metrics (feature rank, dormant neuron fraction, weight norm growth) to identify a minimal recipe that reliably closes the gap to optimal performance across seeds.
The goal is either a concrete configuration that achieves near-100\% success, or evidence that current interventions remain insufficient — pointing to open failure modes.

% Lennart

% \subsubsection{Agentic vs. Classical Hyperparameter Search for Deep RL}
% Karpathy's autoresearch~\cite{karpathy2026autoresearch} suggests frontier coding agents can drive overnight ML research loops, but it offers no controlled comparison against established hyperparameter optimization methods — this project supplies one in the RL setting, where hyperparameter sensitivity and high seed variance make the comparison both meaningful and methodologically demanding. 
% On a fixed RL benchmark (e.g. PPO on a small Gym suite) with identical per-trial and total compute budgets, benchmark a coding agent against random search and Bayesian optimization over the same hyperparameter space. 
% The agent is evaluated in two regimes — restricted to editing a config file, and free to modify the full training code — to disentangle gains from search strategy versus code-editing freedom. 
% The intended contribution is not a winner but a calibrated answer to whether, and under what budget regime, agentic search is actually competitive with classical HPO on noisy RL objectives.
...
\begin{itemize}
  ...
  \item Based on the three-dimensional structure of the robots, design message passing network(s) in addition or instead of the MLP for value/Q function and policy in standard algorithms like PPO \citet{Schulman2017} or SAC \citet{Haarnoja2018a}.
\end{itemize}
...
% Johannes Pitz
\subsubsection{Neural Network-based World Models}

Model-based reinforcement learning promises improved sample efficiency by learning a predictive model of the environment, commonly referred to as a world model. However, accurately capturing long-horizon dynamics in stochastic robotic systems remains a key challenge. In this project, we aim to investigate autoregressive world model training, based on the work of \citet{li2025robotic}. You could, for example:

\begin{itemize}
  \item Reproduce the results from the paper and evaluate the method on additional environments.
  \item Run experiments with different history and forecast horizons to find their influence on error accumulation and generalization.
  \item Investigate robustness under noise and domain shift, and assess implications for sim-to-real transfer.
\end{itemize}
...
\citet{chou2023diffusion} use a variational auto-encoder to find a latent parametrization of 3D geometries using an implicit surface representation.
\citet{luo2021diffusion} find a latent representation by training a point cloud diffusion model conditioned on a latent code generated by an encoder.
\begin{itemize}
  \item Start off with a simple 3D data set (e.g. single category of Shapenet)
  \item Implement an auto-encoder architecture to find a latent representation of shapes.
  \item Compare the implicit surface representation to point clouds. What is more accurate? What is easier to train?
  \item Use the trained auto-encoder for downstream tasks:
  \begin{itemize}
    \item Train a diffusion network to generate novel shapes.
    \item Train a conditioned diffusion network to generate plausible shapes for a given grasp with a robotic hand.
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  For instance, in \citet{zobeidi2021deep} they learn a directional distance function that could replace the raycasting.
